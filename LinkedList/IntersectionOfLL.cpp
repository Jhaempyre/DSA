...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        //Let's find intersectin here the merging/intersection means they share same memory address afterward ;
        int diff = diffrenceofsize(headA,headB);
        cout<<diff;
        if(diff<0){
            while(diff!=0){
                headB=headB->next;
    ...
    int diffrenceofsize(ListNode* a , ListNode* b ){
        int len1 = 0, len2 = 0;
